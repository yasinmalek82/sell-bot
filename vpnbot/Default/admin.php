...
if ($text == "🏪 نام فروشگاه") {
    sendmessage($from_id, "نام نمایشی فروشگاه را ارسال کنید.", $backadmin, 'HTML');
    step('reseller_store_name', $from_id);
} elseif ($user['step'] == 'reseller_store_name') {
    $setting['store_name'] = mb_substr(trim($text), 0, 64);
    if ($setting['store_name'] === '') {
        sendmessage($from_id, "❌ نام فروشگاه نمی‌تواند خالی باشد.", $backadmin, 'HTML');
        return;
    }
    update("botsaz", "setting", json_encode($setting, JSON_UNESCAPED_UNICODE), "bot_token", $ApiToken);
    telegram('setMyName', ['name' => $setting['store_name']]);
    step('home', $from_id);
    sendmessage($from_id, "✅ نام فروشگاه ذخیره شد.", $keyboardadmin, 'HTML');
} elseif ($text == "👋 متن خوش‌آمدگویی") {
    sendmessage($from_id, "متن خوش‌آمد را ارسال کنید. می‌توانید از {name} و {store} استفاده کنید.", $backadmin, 'HTML');
    step('reseller_welcome_text', $from_id);
} elseif ($user['step'] == 'reseller_welcome_text') {
    $setting['welcome_text'] = mb_substr(trim($text), 0, 1500);
    update("botsaz", "setting", json_encode($setting, JSON_UNESCAPED_UNICODE), "bot_token", $ApiToken);
    step('home', $from_id);
    sendmessage($from_id, "✅ متن خوش‌آمد ذخیره شد.", $keyboardadmin, 'HTML');
} elseif ($text == "🧾 متن درباره فروشگاه") {
    sendmessage($from_id, "توضیح کوتاه فروشگاه، خدمات یا راه‌های ارتباطی را ارسال کنید.", $backadmin, 'HTML');
    step('reseller_about_text', $from_id);
} elseif ($user['step'] == 'reseller_about_text') {
    $setting['about_text'] = mb_substr(trim($text), 0, 512);
    update("botsaz", "setting", json_encode($setting, JSON_UNESCAPED_UNICODE), "bot_token", $ApiToken);
    telegram('setMyDescription', ['description' => $setting['about_text']]);
    telegram('setMyShortDescription', ['short_description' => mb_substr($setting['about_text'], 0, 120)]);
    step('home', $from_id);
    sendmessage($from_id, "✅ متن معرفی فروشگاه ذخیره شد.", $keyboardadmin, 'HTML');
    ...
    $supportUsername = ltrim(trim($text), '@');
    if (!preg_match('/^[A-Za-z0-9_]{5,32}$/', $supportUsername)) {
        sendmessage($from_id, "❌ نام کاربری معتبر تلگرام را بدون لینک ارسال کنید.", $backadmin, 'HTML');
        return;
    }
...
👤 ایدی عددی کاربر: {$Payment_report['id_user']}";
    ...
    $__q23->execute();
    $invoicecount = $__q23->fetch(PDO::FETCH_ASSOC)['count(*)'];
    if ($invoicecount == 0) {
