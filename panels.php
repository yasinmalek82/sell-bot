...
class ManagePanel
{
    ...
    function createUser($name_panel, $code_product, $usernameC, array $Data_Config)
    {
        ...
        if (!in_array($code_product, ["usertest", $textbotlang['extracted']['index_php']['customVolumeButton'], "customvolume"])) {
            ...
            if (!$Get_Data_Product) {
                return [
                    'status' => 'Unsuccessful',
                    'msg' => 'Product not found for the selected panel.',
                ];
            }
        } else {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            $ip_limit = isset($Get_Data_Product['ip_limit']) ? intval($Get_Data_Product['ip_limit']) : 0;
            $data_Output = addClient($Get_Data_Panel, $usernameC, $expire, $subId, $data_limit, $inbounds, $Get_Data_Product['name_product'], $note, $ip_limit);
            if (!empty($data_Output['error'])) {
                ...
                if (!is_array($data_Output)) {
                    $Output['status'] = 'Unsuccessful';
                    $Output['msg'] = 'Invalid JSON response from 3x-ui.';
                } elseif (empty($data_Output['success'])) {
                    $Output['status'] = 'Unsuccessful';
                    $Output['msg'] = $data_Output['msg'] ?? '3x-ui rejected the client.';
                } else {
    ...
    function DataUser($name_panel, $username)
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            if (empty($user_data['client']) || !is_array($user_data['client'])) {
                return [
                    'status' => 'Unsuccessful',
                    'msg' => 'Invalid client object returned by 3x-ui.',
                ];
            }
            $xuiClient = $user_data['client'];
            $expire = ((int) ($xuiClient['expiryTime'] ?? 0)) / 1000;
            ...
            if (!is_array($used_data_3xui) || empty($used_data_3xui['success']) || !isset($used_data_3xui['obj'])) {
                return array(
                    'status' => 'Unsuccessful',
                    'msg' => $used_data_3xui['msg'] ?? 'Invalid traffic response from 3x-ui.'
                );
            }
            if (!empty($xuiClient['enable'])) {
                $status = 'active';
            } else {
                $status = 'disabled';
            }
            $usedTraffic = (int) ($used_data_3xui['obj']['up'] ?? 0) + (int) ($used_data_3xui['obj']['down'] ?? 0);
            if ((int) ($xuiClient['totalGB'] ?? 0) !== 0) {
                if (((int) $xuiClient['totalGB'] - $usedTraffic) <= 0) {
                    $status = 'limited';
                }
            }
            if ((int) ($xuiClient['expiryTime'] ?? 0) !== 0 && $expire - time() <= 0) {
                $status = 'expired';
            }
            if ((int) ($xuiClient['expiryTime'] ?? 0) < -10000) {
                $status = 'on_hold';
                $expire = 0;
            }
            $linksub = rtrim($Get_Data_Panel['linksubx'], '/') . '/' . ($xuiClient['subId'] ?? '');
            $lastOnline = (int) ($used_data_3xui['obj']['lastOnline'] ?? 0);
            $lastOnline = $lastOnline === 0 ? 'offline' : date('Y-m-d H:i:s', $lastOnline / 1000);
            ...
            $Output = array(
                'status' => $status,
                'username' => $xuiClient['email'],
                'data_limit' => (int) ($xuiClient['totalGB'] ?? 0),
                'expire' => $expire,
                ...
                'ip_limit' => normalizeXuiIpLimit($xuiClient['limitIp'] ?? 0),
    ...
    function Modifyuser($username, $name_panel, $config = array())
    {
        ...
        if ($Get_Data_Panel['type'] == "marzban") {
            ...
            $data_user = $this->DataUser($name_panel, $username);
            if (($data_user['status'] ?? 'Unsuccessful') === 'Unsuccessful' || empty($data_user['_xui_client'])) {
                return [
                    'status' => false,
                    'msg' => $data_user['msg'] ?? 'Unable to load the current 3x-ui client.',
                ];
            }
            // The 3x-ui update endpoint binds a full Client object. Start from the
            // current object so renew/disable/revoke never clears subId or limitIp.
            $data = buildXuiUpdateClientPayload($data_user['_xui_client'], $config, $username);
            ...
            if (!is_array($modify) || empty($modify['success'])) {
                return array(
                    'status' => false,
                    'msg' => 'error: ' . ($modify['msg'] ?? 'Invalid JSON response from 3x-ui.')
                );
            }
            // update/{email} propagates to all currently attached inbounds.
            // Do not attach panel defaults here: a product may use a smaller,
            // explicitly selected inbound set that must remain unchanged.
            return array(
                'status' => true,
                'data' => $modify
            );
