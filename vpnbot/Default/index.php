...
if ($text == "/start") {
    $storeName = trim((string) ($setting['store_name'] ?? 'فروشگاه ما'));
    $welcomeTemplate = trim((string) ($setting['welcome_text'] ?? 'سلام {name} عزیز 👋\nبه {store} خوش آمدید.'));
    $textstart = strtr($welcomeTemplate, [
        '{name}' => htmlspecialchars((string) $first_name, ENT_QUOTES, 'UTF-8'),
...
📆 $dateacc → ⏰ $timeacc";
    ...
    Editmessagetext($from_id, $message_id, "🛍 برای مشاهده اطلاعات سرویس خود از لیست زیر سرویس خود را انتخاب نمایید", $keyboard_json);
} elseif ($text == ($text_bot_var['btn_keyboard']['about'] ?? '🏪 درباره فروشگاه')) {
    $storeName = htmlspecialchars((string) ($setting['store_name'] ?? 'فروشگاه ما'), ENT_QUOTES, 'UTF-8');
    $aboutText = htmlspecialchars((string) ($setting['about_text'] ?? 'ارائه سرویس با پشتیبانی مستقیم فروشگاه.'), ENT_QUOTES, 'UTF-8');
    sendmessage($from_id, "🏪 <b>{$storeName}</b>\n\n{$aboutText}", $keyboard, 'HTML');
    return;
} elseif ($text == $text_bot_var['btn_keyboard']['support']) {
    $supportUsername = ltrim(trim((string) ($setting['support_username'] ?? '')), '@');
    if (!preg_match('/^[A-Za-z0-9_]{5,32}$/', $supportUsername)) {
        sendmessage($from_id, "☎️ پشتیبانی این فروشگاه هنوز تنظیم نشده است.", $keyboard, 'HTML');
        return;
    }
    ...
    $Keyboardsupport = json_encode([
        'inline_keyboard' => [
            [
                ['text' => "📞 ارتباط با پشتیبانی", 'url' => 'https://t.me/' . $supportUsername, 'style' => 'primary'],
            ],
        ]
    ]);
