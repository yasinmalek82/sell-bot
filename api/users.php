...
switch ($data['actions'] ?? '') {
    ...
    case 'active_bot_agent':
        ...
        try {
            $provisioner = new ResellerBotService($pdo, dirname(__DIR__), $domainhosts);
            $provisioner->provision((string) $data['chat_id'], (string) $data['token'], 'legacy');
        } catch (Throwable $e) {
            error_log('Reseller bot provisioning failed: ' . $e->getMessage());
            sendJsonResponse(false, 'Bot could not be provisioned: ' . $e->getMessage(), [], 500);
        }
        sendJsonResponse(true, "Successful");
        break;
